...
export default function ClanChatScreen({ navigation, route }: ClanChatScreenProps) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const { data } = await clanApi.getMessages(clanId);
      const msgs: ChatMessage[] = (data.data?.messages ?? data.messages ?? []).map((m: any) => ({
        id: m.id,
        clanId: m.clan_id ?? m.clanId ?? clanId,
  ...
  useEffect(() => {
    const unsubscribe = mapRaidersWs.on('clan_message', (data: any) => {
      ...
      setMessages((prev) => {
        // Avoid duplicates (could happen if we sent the message ourselves)
  ...
  const loadOlderMessages = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await clanApi.getMessages(clanId, oldestMessage.id);
  ...
  }, [clanId, messages, loadingMore, hasMore]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      await clanApi.sendMessage(clanId, trimmed);
